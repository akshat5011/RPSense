...
def process_frames():
    ...
    try:
        ...
        # If no final result was generated during processing, try to get one
        if frame_processor.postprocessor.frame_buffer:
            aggregated_result, best_frame = frame_processor.postprocessor.get_aggregated_result()
            
            if aggregated_result and best_frame:
                player_move = aggregated_result["final_prediction"]
                game_result = game_engine.play_round(player_move)
                ...
                final_result = {
                    ...
                    "prediction_percentage": aggregated_result["prediction_percentage"],
                    "all_predictions": aggregated_result["all_predictions"],
                    "message": f"Processed {processed_count} frames successfully",
                    "timestamp": time.time(),
                    "game_result": game_result,
                }
                
                print(f"✅ Generated final result from buffer: {final_result['final_prediction']}")
                return jsonify(final_result)
        ...
        fallback_result = {
            ...
            "confidence": 0.0,
            "message": f"No valid gestures detected in {processed_count} frames",
            "timestamp": time.time(),
            "game_result": game_engine.play_round("invalid"),
        }
        
        print(f"⚠️ No valid predictions found, returning fallback result")
        return jsonify(fallback_result)
        
    except Exception as e:
        print(f"❌ HTTP frame processing error: {str(e)}")
